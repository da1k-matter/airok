...
def save_plots(daily: pd.DataFrame, output_dir: Path, prefix: str = "") -> None:
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"{prefix}equity.png", dpi=160)
    plt.close()
    ...
    plt.ylabel("Drawdown")
    plt.tight_layout()
    plt.savefig(output_dir / f"{prefix}drawdown.png", dpi=160)
    plt.close()


def environment_text() -> str:
    packages: dict[str, str] = {}
    for name in ("numpy", "pandas", "scipy", "numba", "lightgbm", "optuna", "yaml", "matplotlib"):
        try:
