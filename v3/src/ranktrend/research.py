...
def train_walk_forward(
    ...
    for fold, pstart in enumerate(starts, start=1):
        ...
        prediction = fit_predict_lightgbm(
            x_all[train_rows],
            y_all[train_rows],
        ...
        LOG.info(
            "lightgbm h%d seed=%d fold=%02d %s..%s train=%s predict=%s",
            spec.horizon,
            seed,
            fold,
            pstart.date(),
...
def blend_horizons(
    ...
    for score, spec in zip(ranked, model_specs, strict=True):
        good = np.isfinite(score)
        output[good] += np.float32(spec.weight) * score[good]
        valid_weight[good] += np.float32(spec.weight)
    return np.divide(
        output,
        valid_weight,
