...
def prediction_cache_file(config: ExperimentConfig, spec: ModelSpec, seed: int) -> Path:
    relevant = {
        "lightgbm": config.section("lightgbm"),
        "data": config.section("data"),
    ...
    return config.cache_dir / "predictions" / f"lightgbm_h{spec.horizon}_s{seed}_{digest}.npz"
...
def train_models(
    ...
    lightgbm_params = config.section("lightgbm").get("params", {})
    ...
    diagnostics.update(
        {
            ...
            "backend": "lightgbm",
            "backend_version": str(getattr(lightgbm, "__version__", "unknown")),
            "horizons": [spec.horizon for spec in model_specs(config)],
            "seeds": [int(seed) for seed in config.section("training")["seeds"]],
