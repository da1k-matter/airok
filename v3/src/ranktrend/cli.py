...
from .tune import evaluate_trial_ensemble, evaluate_trials, run_tuning
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in (
        ...
        command.add_argument("--config", default="configs/lightgbm_h7_h10.yaml", type=Path)
        command.add_argument("--data-dir", type=Path, help="Override data.directory from YAML")
        if name in {"build-features", "train", "backtest", "run", "tune", "evaluate-trials", "evaluate-ensemble"}:
        ...
        if name == "tune":
            command.add_argument("--trials", type=int, help="Target total number of completed trials")
        if name in {"evaluate-trials", "evaluate-ensemble"}:
            command.add_argument("--trial", type=int, action="append", required=True)
    return root
...
def print_summary(summary: dict) -> None:
    model = summary["model"]
    print(
        f"\n{model['backend']} horizons={model['horizons']} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "tune":
            output = run_tuning(config, n_trials=args.trials)
            print(f"tuning outputs: {output}")
            return 0

        if args.command == "evaluate-trials":
            output = evaluate_trials(config, trial_numbers=args.trial)
            print(f"holdout outputs: {output}")
            return 0

        if args.command == "evaluate-ensemble":
            output = evaluate_trial_ensemble(config, trial_numbers=args.trial)
            print(f"ensemble outputs: {output}")
            return 0
