...
def rolling_mean(x: np.ndarray, window: int, min_periods: int | None = None) -> np.ndarray:
    ...
    return pd.DataFrame(x).rolling(window, min_periods=min_periods).mean().to_numpy(dtype=np.float32)
...
def rolling_std(x: np.ndarray, window: int, min_periods: int | None = None) -> np.ndarray:
    ...
    return pd.DataFrame(x).rolling(window, min_periods=min_periods).std(ddof=0).to_numpy(dtype=np.float32)


def rolling_max(x: np.ndarray, window: int, min_periods: int | None = None) -> np.ndarray:
    min_periods = min_periods if min_periods is not None else max(2, window // 2)
    return pd.DataFrame(x).rolling(window, min_periods=min_periods).max().to_numpy(dtype=np.float32)
...
def build_features(panel: dict[str, Any], min_age: int = 180, max_universe: int = 150) -> dict[str, Any]:
    ...
    dvol = c * v
    log_dvol = np.log1p(dvol)
    dvol20 = rolling_mean(log_dvol, 20, 10)
    dvol60 = rolling_mean(log_dvol, 60, 30)
    ...
    btc_r = ret1[:, btc_idx].astype(np.float64)
    btc_r2d = np.broadcast_to(btc_r[:, None], ret1.shape)
    mean_r60 = rolling_mean(ret1, 60, 30)
    mean_b60 = pd.Series(btc_r).rolling(60, min_periods=30).mean().to_numpy()
    mean_rb60 = rolling_mean(ret1 * btc_r2d, 60, 30)
    mean_b2_60 = pd.Series(btc_r * btc_r).rolling(60, min_periods=30).mean().to_numpy()
    ...
    for k in (1, 2, 3, 5, 7, 14, 21, 30, 60, 90, 120):
        ...
        raw[f"ret_{k}"] = x
    for k in (5, 10, 20, 60):
        raw[f"vol_{k}"] = rolling_std(ret1, k)
    neg = np.minimum(ret1, 0.0)
    raw["downvol_20"] = np.sqrt(rolling_mean(neg * neg, 20, 10)).astype(np.float32)
    raw["downvol_60"] = np.sqrt(rolling_mean(neg * neg, 60, 30)).astype(np.float32)
    ...
    raw["close_loc_1"] = (c - l) / denom - 0.5
    raw["close_loc_5"] = rolling_mean(raw["close_loc_1"], 5, 3)
    raw["body_1"] = (c - o) / denom
    raw["body_5"] = rolling_mean(raw["body_1"], 5, 3)

    raw["volume_5_20"] = rolling_mean(log_dvol, 5, 3) - dvol20
    raw["volume_20_60"] = dvol20 - dvol60
    raw["dvol_level"] = dvol20
    raw["amihud_20"] = rolling_mean(np.abs(ret1) / np.maximum(dvol, 1.0), 20, 10)
    for k in (20, 60, 120):
        raw[f"dist_high_{k}"] = c / rolling_max(c, k) - 1.0
    raw["beta_60"] = beta60
    ...
    for name, arr in raw.items():
        features[f"{name}_xrank"] = (rolling_rank_pct(arr, eligible_max) - 0.5).astype(np.float32)
    # Empty early cross-sections are expected before the age warm-up.
    ...
    for name, vec in regimes.items():
        features[name] = np.broadcast_to(vec[:, None], c.shape).astype(np.float32)
...
def panel_to_rows(ctx: dict[str, Any]) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "X": ctx["feature_stack"][date_idx, asset_idx].astype(np.float32),
    }
...
def train_walk_forward(
    ...
    date_rows, asset_rows, x_all = rows["date_idx"], rows["asset_idx"], rows["X"]
    ...
    for fold, pstart in enumerate(starts, start=1):
        ...
        pred_mask = (date_rows >= p0) & (date_rows <= p1)
        train_rows = np.flatnonzero(train_mask)
        pred_rows = np.flatnonzero(pred_mask)
        if train_rows.size < 5000 or pred_rows.size == 0:
            continue
        d_train = date_rows[train_rows]
        model = LGBMRanker(
            ...
            min_child_samples=int(training["min_child_samples"]),
            subsample=float(training["subsample"]),
            colsample_bytree=float(training["colsample_bytree"]),
            reg_alpha=float(training["reg_alpha"]),
            reg_lambda=float(training["reg_lambda"]),
            random_state=int(training["seed"]),
            n_jobs=int(training["n_jobs"]),
            force_col_wise=True,
            max_bin=int(training["max_bin"]),
            verbosity=-1,
        )
        model.fit(
            x_all[train_rows],
            y_all[train_rows].astype(np.int32),
            group=_groups_from_sorted_dates(d_train),
        )
        with warnings.catch_warnings():
            ...
            prediction = model.predict(x_all[pred_rows]).astype(np.float32)
        scores[date_rows[pred_rows], asset_rows[pred_rows]] = prediction
        LOG.info(
            "%s fold %02d: %s..%s train=%s predict=%s",
            spec.name,
            fold,
            pstart.date(),
            pend.date(),
...
def ensemble_scores(score_arrays: list[np.ndarray], weights: list[float], eligible: np.ndarray) -> np.ndarray:
    ranked = [rank_model_scores(score, eligible) for score in score_arrays]
    output = np.zeros_like(ranked[0], dtype=np.float32)
    valid_weight = np.zeros_like(output, dtype=np.float32)
    for score, weight in zip(ranked, weights, strict=True):
        good = np.isfinite(score)
        output[good] += np.float32(weight) * score[good]
        valid_weight[good] += np.float32(weight)
    output = np.divide(output, valid_weight, out=np.full_like(output, np.nan), where=valid_weight > 0)
