...
def yearly_table(
    ...
    for year in sorted(set(dates[overall].year)):
        ...
        rows.append(row)
    columns = ["year", "start", "end", "return", "cagr", "vol", "sharpe", "sortino", "max_dd", "calmar", "positive_months", "n_months", "turnover_pa", "worst_month", "best_month"]
    return pd.DataFrame(rows)[columns]


def save_plots(daily: pd.DataFrame, output_dir: Path) -> None:
    plt.figure(figsize=(11, 6))
    plt.plot(pd.to_datetime(daily["date"]), daily["equity"])
    ...
    plt.plot(pd.to_datetime(daily["date"]), daily["drawdown"])
