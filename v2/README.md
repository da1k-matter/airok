...
The package supports both strategy modes requested:

- **7d** — one forward-ranking model;
- **7d + 10d** — two independently trained rankers, combined 50/50 by daily cross-sectional rank.

It also supports deterministic single-seed runs and multi-seed ensembles without duplicating pipeline code.
...
| Config | Backend | Horizons | Seeds | Seed aggregation |
|---|---|---:|---:|---|
| `configs/lightgbm_h7.yaml` | LightGBM LambdaRank | 7 | 5 | median rank |
| `configs/lightgbm_h7_h10.yaml` | LightGBM LambdaRank | 7 + 10 | 5 | median rank |
| `configs/catboost_h7.yaml` | CatBoost YetiRankPairwise NDCG | 7 | 5 | mean rank |
| `configs/catboost_h7_h10.yaml` | CatBoost YetiRankPairwise NDCG | 7 + 10 | 5 | mean rank |
| `configs/lightgbm_legacy_best_v2.yaml` | LightGBM LambdaRank | 7 + 10 | seed 42 | historical single-seed setup |

The legacy preset preserves the exact shape associated with the historical latest-year Sharpe near 2.36. It is kept as a reference, not selected as the default production configuration.
...
```bash
ranktrend run \
  --config configs/catboost_h7_h10.yaml \
  --data-dir /path/to/bybit/1d
```
...
```bash
RANKTREND_DATA=/path/to/bybit/1d ./run.sh configs/catboost_h7_h10.yaml
```
...
ranktrend doctor --config configs/lightgbm_h7_h10.yaml --data-dir /path/to/1d

# Build or load features
ranktrend build-features --config configs/lightgbm_h7_h10.yaml --data-dir /path/to/1d

# Train all horizon × seed models
ranktrend train --config configs/lightgbm_h7_h10.yaml --data-dir /path/to/1d

# Backtest from cached predictions
ranktrend backtest --config configs/lightgbm_h7_h10.yaml --data-dir /path/to/1d
```
...
For each seed, horizons are blended first:

```text
seed signal = 0.5 × rank(score_7d) + 0.5 × rank(score_10d)
```

Then seeds are combined using one configured method:
...
Historical checkpoints are retained in `reference/benchmarks.json`; they are documentation, not hard cross-platform assertions.
